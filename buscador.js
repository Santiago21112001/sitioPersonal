function search() {
  ...
  function searchInPage(pageName, content) {
    try {
      ...
      $(content)
        .find("h1, h2, h3, h4, h5, h6, p, a, ul li, ol li")
        .each(function () {
      ...
      results.push(
        `<p><strong>Error al procesar la página ${pageName}.</strong></p>`
      );
    }
  }

  // Lista de todas las páginas del sitio para buscar coincidencias
  const allPages = [
    "index.html",
    "hobbies.html",
    "computing.html",
  ...
  function searchInOtherPages(pageName) {
    return fetch(pageName) // Cargar el contenido del archivo HTML
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Error al cargar ${pageName}: ${response.statusText}`
          );
        }
      ...
      .catch((error) => {
        console.error(error);
        results.push(
          `<p><strong>No se pudo cargar el archivo ${pageName}: ${error.message}</strong></p>`
        );
      });
  }

  // Ejecutar la búsqueda en las otras páginas del sitio
  const promises = allPages.map((pageName) => searchInOtherPages(pageName));
  ...
  Promise.all(promises)
    ...
    .catch((error) => {
      ...
      results.push(
        "<p><strong>Hubo un error en la búsqueda. Por favor, intente de nuevo más tarde.</strong></p>"
      );
